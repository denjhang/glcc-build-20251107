...
// Explicit declarations for functions used in TSTblit.c
static unsigned int makew(unsigned char h, unsigned char l) {
    return (h << 8) | l;
}
void SYS_Fill(unsigned int addr, int color, unsigned int size) {
    // 简化的SYS_Fill实现
...
void SYS_Blit(unsigned int dst_addr, unsigned int src_addr, unsigned int size) {
    // 简化的SYS_Blit实现
    volatile unsigned char *dst = (volatile unsigned char *)dst_addr;
    volatile unsigned char *src = (volatile unsigned char *)src_addr;
    unsigned int i;
    for (i = 0; i < size; i++) {
        dst[i] = src[i];
    }
