...
static void erase_sprite_trail(void)
{
    ...
    if (sprite_x > prev_sprite_x) {
        /* 向右移动，擦除左边缘 */
        for (i = 0; i < 6; i++) {
            if (prev_sprite_y + i >= DRAW_START_Y && prev_sprite_y + i < HEIGHT - BORDER) {
                screenMemory[prev_sprite_y + i][prev_sprite_x] = 0x3F; /* 白色 */
            }
        }
        ...
        for (i = 0; i < 6; i++) {
            if (prev_sprite_y + i >= DRAW_START_Y && prev_sprite_y + i < HEIGHT - BORDER) {
                screenMemory[prev_sprite_y + i][prev_sprite_x + 5] = 0x3F; /* 白色 */
            }
        }
    } else if (sprite_y > prev_sprite_y) {
        /* 向下移动，擦除上边缘 */
        for (i = 0; i < 6; i++) {
            if (prev_sprite_x + i >= DRAW_START_X && prev_sprite_x + i < WIDTH - BORDER) {
                screenMemory[prev_sprite_y][prev_sprite_x + i] = 0x3F; /* 白色 */
            }
        }
    } else if (sprite_y < prev_sprite_y) {
        /* 向上移动，擦除下边缘 */
        for (i = 0; i < 6; i++) {
            if (prev_sprite_x + i >= DRAW_START_X && prev_sprite_x + i < WIDTH - BORDER) {
                screenMemory[prev_sprite_y + 5][prev_sprite_x + i] = 0x3F; /* 白色 */
            }
        }
    }
}
