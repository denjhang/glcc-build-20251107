...
/* 4个方向的擦除精灵数据 - 用于擦除精灵移动轨迹 */
static char erase_left_sprite[36];   /* 向左移动时擦除右侧一列 */
static char erase_right_sprite[36];  /* 向右移动时擦除左侧一列 */
static char erase_up_sprite[36];     /* 向上移动时擦除下方一行 */
static char erase_down_sprite[36];   /* 向下移动时擦除上方一行 */
...
/* 初始化擦除精灵数据 */
static void init_erase_sprites(void)
{
    int i, j;
    
    /* 初始化所有精灵为全透明（0） */
    for (i = 0; i < 36; i++) {
        erase_left_sprite[i] = 0;
        erase_right_sprite[i] = 0;
        erase_up_sprite[i] = 0;
        erase_down_sprite[i] = 0;
    }
    
    /* 向左移动时擦除右侧一列（第5列，索引5） */
    for (i = 0; i < 6; i++) {
        erase_left_sprite[i * 6 + 5] = 0x3F;  /* 白色 */
    }
    
    /* 向右移动时擦除左侧一列（第0列，索引0） */
    for (i = 0; i < 6; i++) {
        erase_right_sprite[i * 6 + 0] = 0x3F;  /* 白色 */
    }
    
    /* 向上移动时擦除下方一行（第5行，索引30-35） */
    for (j = 0; j < 6; j++) {
        erase_up_sprite[30 + j] = 0x3F;  /* 白色 */
    }
    
    /* 向下移动时擦除上方一行（第0行，索引0-5） */
    for (j = 0; j < 6; j++) {
        erase_down_sprite[j] = 0x3F;  /* 白色 */
    }
}
...
/* 使用SYS_Sprite6绘制精灵 - 在指定位置 */
static void draw_sprite_at_position(int x, int y, char* sprite_data)
{
    /* 确保精灵在绘制区域内 */
    if (x < DRAW_START_X) x = DRAW_START_X;
    if (y < DRAW_START_Y) y = DRAW_START_Y;
    if (x >= WIDTH - BORDER - 6) x = WIDTH - BORDER - 6;
    if (y >= HEIGHT - BORDER - 6) y = HEIGHT - BORDER - 6;
...
/* 使用SYS_Sprite6绘制精灵 - 在当前位置（使用默认精灵数据） */
static void draw_sprite_at_current_position(void)
{
    draw_sprite_at_position(sprite_x, sprite_y, test_sprite);
}

/* 绘制擦除精灵 - 在指定位置 */
static void draw_erase_sprite(int x, int y, char* erase_sprite)
{
    draw_sprite_at_position(x, y, erase_sprite);
}
...
static void generate_and_draw_sprite(void)
{
    ...
    /* 绘制精灵 */
    draw_sprite_at_current_position();
}
...
static void execute_key_action(void)
{
    ...
    if(!(buttonState & KEY_LEFT)) {
        if (sprite_x > DRAW_START_X) {
            /* 先擦除右侧轨迹 */
            draw_erase_sprite(sprite_x, sprite_y, erase_left_sprite);
            /* 移动精灵 */
            sprite_x--;
            /* 绘制精灵 */
            draw_sprite_at_current_position();
    ...
    if(!(buttonState & KEY_RIGHT)) {
        if (sprite_x < WIDTH - BORDER - 6) {
            /* 先擦除左侧轨迹 */
            draw_erase_sprite(sprite_x, sprite_y, erase_right_sprite);
            /* 移动精灵 */
            sprite_x++;
            /* 绘制精灵 */
            draw_sprite_at_current_position();
    ...
    if(!(buttonState & KEY_UP)) {
        if (sprite_y > DRAW_START_Y) {
            /* 先擦除下方轨迹 */
            draw_erase_sprite(sprite_x, sprite_y, erase_up_sprite);
            /* 移动精灵 */
            sprite_y--;
            /* 绘制精灵 */
            draw_sprite_at_current_position();
    ...
    if(!(buttonState & KEY_DOWN)) {
        if (sprite_y < HEIGHT - BORDER - 6) {
            /* 先擦除上方轨迹 */
            draw_erase_sprite(sprite_x, sprite_y, erase_down_sprite);
            /* 移动精灵 */
            sprite_y++;
            /* 绘制精灵 */
            draw_sprite_at_current_position();
